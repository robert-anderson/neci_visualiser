...
class SVG_renderer:
    ...
    def add_heatmap(self, left, bottom, square_size, data, pinned_values, colour_list):
        sh = colours.SmoothHue(pinned_values, colour_list)
        for i in range(data.shape[0]):
            for j in range(data.shape[1]):
                self.add_square(left+j*square_size, bottom-i*square_size, square_size, colour=sh.get_colour(data[i,j]))
...
print matrix
sr.add_heatmap(100,500, 50, matrix, [colours.red, colours.white, colours.blue])
sr.render()
